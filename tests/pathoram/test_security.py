...
# def test_bitflip(oram_instance_specific):
#     '''
...
#     # retrieve encrypted block from server queue
#     # ctext: client_id || W || leaf_node || nonce || encr data
#     cid = oram_instance_specific.client_oram.client_id

#     block = oram_instance_specific.server_last_queue_obj
#     nonce = block[len(cid)+1:len(cid)+1+12]
#     ciphertext = block[len(cid)+1+12:]
#     assert len(ciphertext) == 64
#     plaintext = oram_instance_specific.client_oram.aes.decrypt(nonce, ciphertext, associated_data=None)
